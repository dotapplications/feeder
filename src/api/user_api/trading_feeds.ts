...
const tradingFeedsCollection = firestore.collection("trading_feeds");

// add a new trading feed
export const addTradingFeed = async (headline: any) => {
  try {
    var feed = {
      headline: headline,
    ...
    await tradingFeedsCollection.add(feed);
    return "Trading feed added successfully";
  } catch (error) {
    console.error("Error adding trading feed:", error);
    throw error;
  }
};
